...
6. **🎯 The COD Score** — five classifiers trained, the calibrated one becomes the score
7. **⚖️ The Verdict** — interactive policy sliders, ₹ trade-off math, recommendations
8. **🧩 Segments** — unsupervised learning: K-Means (elbow + silhouette + interactive 3D)
   and a Gaussian-Mixture latent-class-style model
9. **💰 Order Value** — regression (Linear, Ridge, Lasso, Decision Tree) predicting basket size
10. **🔗 Combos** — association rule mining (support, confidence, lift) on risky trait combinations
