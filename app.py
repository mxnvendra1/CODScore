...
def _find_csv():
    """Locate cod_orders.csv wherever it was uploaded: next to app.py, in data/,
    in the working directory, or anywhere in the repo tree."""
    base = Path(__file__).resolve().parent
    for c in [base / "cod_orders.csv", base / "data" / "cod_orders.csv",
              Path.cwd() / "cod_orders.csv", Path.cwd() / "data" / "cod_orders.csv"]:
        if c.exists():
            return str(c)
    hits = sorted(base.glob("**/cod_orders*.csv")) or sorted(base.glob("**/*orders*.csv"))
    return str(hits[0]) if hits else None

@st.cache_data(show_spinner=False)
def load_raw(file_bytes, path=None):
...
st.sidebar.markdown("""
1. 📦 **Overview** — the problem: why COD hurts
2. 🧾 **Data** — our store
3. 🧹 **Data Preparation** — cleaned & logged
4. 📊 **Descriptive Analytics** — who returns
5. 🔍 **Diagnostic Analytics** — the real reasons
6. 🎯 **Classification** — the COD Score
7. ⚖️ **Prescriptive** — the verdict & policy
8. 🧩 **Clustering** — shopper segments
9. 💰 **Regression** — what drives basket size
10. 🔗 **Association Rules** — risky combos
""")
...
_csv_path = None
if _up_bytes is None:
    _csv_path = _find_csv()
    if _csv_path is None:
        st.error(
            "**Dataset not found.** The app looked for `cod_orders.csv` next to "
            "`app.py`, in a `data/` folder, and everywhere in the repository — "
            "it isn't there.\n\n"
            "**Fix:** on GitHub open your repo → **Add file → Upload files** → drag "
            "in `cod_orders.csv` (repo root is fine — the app will find it) → "
            "**Commit changes**. Then reboot from **Manage app → ⋮ → Reboot app**.\n\n"
            "Or use the sidebar's **'Use your own CSV'** uploader as a stop-gap.")
        st.stop()
raw = load_raw(_up_bytes, _csv_path)
...
tabs = st.tabs([
    "1 · 📦 Overview", "2 · 🧾 Data", "3 · 🧹 Data Preparation",
    "4 · 📊 Descriptive Analytics", "5 · 🔍 Diagnostic Analytics",
    "6 · 🎯 Classification", "7 · ⚖️ Prescriptive · Verdict",
    "8 · 🧩 Clustering", "9 · 💰 Regression", "10 · 🔗 Association Rules",
])
